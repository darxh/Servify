...
const CreateServicePage = () => {
  ...
  const detectLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        try {
          ...
          if (data && data.display_name) {
            ...
            setLocationMessage({ type: "success", text: "Location detected automatically!" });
          }
        } catch (error) {
      ...
      (error) => {
        setLocationLoading(false);
        setLocationMessage({ type: "error", text: "Please allow location permissions in your browser." });
      }
    );
  };
  ...
  const onSubmit = (data) => {
    if (!coordinates.lat || !coordinates.lng) {
      setLocationMessage({ type: "error", text: "You must verify your location or use auto-detect before publishing." });
      return;
    }
